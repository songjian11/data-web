 ...
 */
@Repository
public class AreaPFBattleDao extends AbstractDao {
	private static final RowMapper<BaseReport> BASE_REPORT_RM = new BeanPropertyRowMapper<>(BaseReport.class);
	// 当日实时客流列表
	private static final String GET_TODAY_PF_LIST = "select sale.SALE_HOUR AS time,store.STORE_ID as org_id,store.STORE_NAME as org_name, "
			...
			+ "where to_char(sysdate,'yyyymmdd')=sale.SALE_DATE  and sale.COM_ID='ALL' " + "and store.AREA_ID=?";

	// 当月历史客流列表
	private static final String GET_CURMONTH_PF_LIST = "select to_char(history.SALE_DATE,'yyyy-mm-dd') AS time,history.KL AS value, "
			+ "store.STORE_ID as org_id,store.STORE_NAME as org_name,store.STORE_ID ,store.IS_COMPARE "
	...
	private static final String GET_LASTYEARDAY_C_PF_TOTAL = "select NVL(sum(KL),0) from CSMB_COM_KL_HISTORY history "
			+ "left join CSMB_STORE store on history.STORE_ID=store.STORE_ID "
			+ "where to_char(history.SALE_DATE,'yyyy-mm-dd')=to_char(ADD_MONTHS(sysdate, -12),'yyyy-mm-dd') "
			+ "and store.AREA_ID=? and history.COM_ID='ALL' and store.IS_COMPARE=1";
	...
	private static final String GET_LASTYEAR_DAY_LIST = "SELECT  /*+PARALLEL(8)*/  s.store as org_id "
			+ ",s.store_name as org_name,count(distinct tran_seq_no) as value "
	...
	private static final String GET_LASTYEAR_MONTH_LIST = "SELECT  /*+PARALLEL(8)*/  s.store as org_id "
			...
			+ " ) and a.store = b.store_id and a.item=c.item  and a.store = s.store "
			+ "  and b.dept_id=c.dept  and s.region = ?  and (b.com_id<>'ALL' or b.com_name='整店')  " + "   group by s.store,s.store_name "
			+ "";
	 ...
	 * @throws Exception
	 */
	public List<BaseReport> getTodayPFList(String areaId) throws Exception {
		return super.queryForList(GET_TODAY_PF_LIST, BASE_REPORT_RM, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public List<BaseReport> getCurMonthPFList(String areaId) throws Exception {
		return super.queryForList(GET_CURMONTH_PF_LIST, BASE_REPORT_RM, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastYearDayPFTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastYearDayCPFTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastMonthDayPFTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastYearMonthPFTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastYearMonthCPFTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastMonthPFTotal(String areaId) throws Exception {
		Object[] args = { areaId };
