 ...
 */
@Repository
public class AreaGMBattleDao extends AbstractDao {
	private static final RowMapper<BaseReport> BASE_REPORT_RM = new BeanPropertyRowMapper<>(BaseReport.class);
	// 当日实时总毛利列表
	private static final String GET_TODAY_GM_LIST = "select sale.SALE_HOUR AS time,store.store_id as org_id,store.STORE_NAME  as org_name, "
			...
			+ "where to_char(sysdate,'yyyymmdd')=sale.SALE_DATE and store.AREA_ID=?";
	// 当日实时前台毛利列表
	private static final String GET_TODAY_FGM_LIST = "select sale.SALE_HOUR AS time,store.STORE_ID as org_id,store.STORE_NAME as org_name, "
			+ "NVL(sale.GP_ECL,0) AS value,store.STORE_ID,store.IS_COMPARE from CSMB_COM_SALES sale "
			+ "left join CSMB_STORE store on sale.STORE_ID=store.STORE_ID "
			+ "where to_char(sysdate,'yyyymmdd')=sale.SALE_DATE and store.AREA_ID=? ";

	// 当月历史毛利列表
	private static final String GET_CURMONTH_GM_LIST = "select to_char(history.SALE_DATE,'yyyy-mm-dd') AS time,store.STORE_ID as org_id, "
			...
			+ ",store.STORE_NAME as org_name,store.STORE_ID,store.IS_COMPARE "
			...
			+ "where to_char(sysdate,'yyyy-mm')=to_char(history.SALE_DATE,'yyyy-mm') and store.AREA_ID=?";
	...
	private static final String GET_LASTYEARDAY_C_GM_TOTAL = "select NVL(sum((history.SALE_VALUE-history.SALE_COST+history.INVADJ_COST-history.WAC+history.FUND_AMOUNT) "
			...
			+ "on history.DEPT_ID=income.DEPT_ID "
			+ "where to_char(history.SALE_DATE,'yyyy-mm-dd')=to_char(ADD_MONTHS(sysdate, -12),'yyyy-mm-dd') "
			+ "and store.AREA_ID=? and store.IS_COMPARE=1";
	...
	private static final String GET_LASTYEARMONTH_C_GM_TOTAL = "select "
			...
			+ "on history.DEPT_ID=income.DEPT_ID "
			+ "where to_char(history.SALE_DATE,'yyyy-mm')=to_char(ADD_MONTHS(sysdate, -12),'yyyy-mm') "
			+ "and store.AREA_ID=1 and store.IS_COMPARE=1";
	...
	// 去年今日列表
	private static final String GET_LASTYEAR_DAY_LIST = "SELECT  /*+PARALLEL(8)*/  s.store as org_id "
			+ ",s.store_name as org_name,sum(a.total_real_amt / (1 + a.sale_vat_rate / 100) - a.unit_cost_ecl * a.qty) as value "
			...
			+ "  and b.dept_id=c.dept  and s.region = ? " + "	 and (b.com_id<>'ALL' or b.com_name='整店')  "
			+ "   and tran_datetime<=TO_CHAR(SYSDATE,'HH24:MI:SS')  group by s.store,s.store_name";

	// 去年本月列表
	private static final String GET_LASTYEAR_MONTH_LIST = "SELECT  /*+PARALLEL(8)*/  s.store as org_id "
			+ ",s.store_name as org_name,sum(a.total_real_amt / (1 + a.sale_vat_rate / 100) - a.unit_cost_ecl * a.qty) as value "
	 ...
	 * @throws Exception
	 */
	public List<BaseReport> getTodayGMList(String areaId) throws Exception {
		return super.queryForList(GET_TODAY_GM_LIST, BASE_REPORT_RM, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public List<BaseReport> getTodayFGMList(String areaId) throws Exception {
		return super.queryForList(GET_TODAY_FGM_LIST, BASE_REPORT_RM, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public List<BaseReport> getCurMonthGMList(String areaId) throws Exception {
		return super.queryForList(GET_CURMONTH_GM_LIST, BASE_REPORT_RM, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastYearDayGMTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastYearDayCGMTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastMonthDayGMTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastYearMonthGMTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastYearMonthCGMTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @throws Exception
	 */
	public BigDecimal getLastMonthGMTotal(String areaId) throws Exception {
		Object[] args = { areaId };
	 ...
	 * @date 2019年4月5日
	 */
	public List<BaseReport> getLastYearDayList(String areaId) throws Exception {
		return super.queryForList(GET_LASTYEAR_DAY_LIST, BASE_REPORT_RM, areaId);
	}
	 ...
	 * @date 2019年4月5日
	 */
	public List<BaseReport> getLastYearMonthList(String areaId) throws Exception {
		return super.queryForList(GET_LASTYEAR_MONTH_LIST, BASE_REPORT_RM, areaId);
	}
