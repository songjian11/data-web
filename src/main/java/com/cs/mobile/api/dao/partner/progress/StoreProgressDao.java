 ...
 */
@Repository
public class StoreProgressDao extends AbstractDao {
	...
	private static final String GET_TODAY_DEPT_SALE = "select sale.SALE_HOUR AS time,sale.DEPT_ID as org_id,sale.DEPT_ID as org_name, "
			...
			+ "where to_char(sysdate,'yyyymmdd')=sale.SALE_DATE and (company.COM_ID<>'ALL' or company.COM_NAME='整店')  and store.AREA_ID=?";
	...
	private static final String GET_HISTORY_DEPT_SALE = "select to_char(history.SALE_DATE,'yyyy-mm-dd') AS time,history.SALE_VALUE AS value,company.DEPT_ID as org_id "
			...
			+ "and to_char(history.SALE_DATE,'yyyy-mm-dd')<=?  and (company.COM_ID<>'ALL' or company.COM_NAME='整店') and store.AREA_ID=?";
	// 小店分配比例
	private static final String GET_COM_DISTRIBUTION_CNF = "select g.store_id,d.com_id,d.D_VALUES as value "
			...
			+ "left join CSMB_STORE store on g.STORE_ID=store.STORE_ID where store.AREA_ID=?";
	// 查询大类库存
	private static final String GET_DEPT_STOCK = "select stock.*,company.com_id from CSMB_DEPT_STOCK stock "
			...
			+ "where to_char(STOCK_DATE,'yyyy-mm-dd')=? and store.AREA_ID=? and (company.COM_ID<>'ALL' or company.COM_NAME='整店')";
	// 查询大类标准周转天数
	private static final String GET_DEPT_TURNOVER_DAYS = "select td.* from CSMB_TURNOVER_DAYS td "
			+ "left join CSMB_STORE store on td.STORE_ID=store.STORE_ID where t_ym=? " + "and store.AREA_ID=? ";
	 ...
	 * @throws Exception
	 */
	public List<BaseReportGpExt> getTodayDeptSale(String areaId) throws Exception {
		return super.queryForList(GET_TODAY_DEPT_SALE, BASE_REPORT_GP_EXT_RM, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public List<BaseReportGpExt> getHistoryDeptSale(String areaId, String beginYmd, String endYmd) throws Exception {
		return super.queryForList(GET_HISTORY_DEPT_SALE, BASE_REPORT_GP_EXT_RM, beginYmd, endYmd, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public List<DistributionValues> getComDistributionCnf(String areaId) throws Exception {
		return super.queryForList(GET_COM_DISTRIBUTION_CNF, DISTRIBUTION_VALUES_RM, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public List<DeptStock> getDeptStock(String areaId, String ymd) throws Exception {
		return super.queryForList(GET_DEPT_STOCK, DEPT_STOCK_RM, ymd, areaId);
	}
	 ...
	 * @throws Exception
	 */
	public List<TurnOverDays> getDeptTurnoverDays(String areaId, String ym) throws Exception {
		return super.queryForList(GET_DEPT_TURNOVER_DAYS, TURN_OVERDAYS_RM, ym, areaId);
	}
}
