 ...
 */
@Repository
public class AreaSaleBattleDao extends AbstractDao {
	private static final RowMapper<BaseReport> BASE_REPORT_RM = new BeanPropertyRowMapper<>(BaseReport.class);
	// 当日实时销售列表
	private static final String GET_TODAY_SALE_LIST = "select sale.SALE_HOUR AS time,store.STORE_ID as org_id,store.STORE_NAME as org_name, "
			+ "store.STORE_ID,store.IS_COMPARE, NVL(sale.AMT_ECL,0) AS value from CSMB_COM_SALES sale "
			...
			+ "where to_char(sysdate,'yyyymmdd')=sale.SALE_DATE  and store.AREA_ID=? and (company.COM_ID<>'ALL' or company.COM_NAME='整店') ";

	// 当月历史销售列表
	private static final String GET_CURMONTH_SALE_LIST = "select to_char(history.SALE_DATE,'yyyy-mm-dd') AS time,history.SALE_VALUE AS value,store.STORE_ID as org_id "
	...
	private static final String GET_LASTYEAR_DAY_LIST = "SELECT  /*+PARALLEL(8)*/  s.store as org_id "
			+ ",s.store_name as org_name,sum(a.total_real_amt / (1 + a.sale_vat_rate / 100)) as value "
