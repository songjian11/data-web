...
import com.cs.mobile.api.dao.partner.assess.ComAssessDao;
import com.cs.mobile.api.dao.partner.goal.GoalDao;
import com.cs.mobile.api.dao.partner.progress.ComProgressDao;
...
@Service
@Slf4j
public class ComProgressServiceImpl extends ProgressAbstractService implements ComProgressService {
	@Autowired
	CommonDao commonDao;
	@Autowired
	GoalDao goalDao;
	@Autowired
	ComProgressDao comProgressDao;
	@Autowired
	ComAssessDao comAssessDao;
	@Autowired
	 ...
	 * @throws Exception
	 */
	public ProgressReportResult getProgressReport(String groupId, String storeId, String comId, String beginYmd,
			String endYmd) throws Exception {
		if (StringUtils.isEmpty(comId)) {// 当大店进入的时候以后勤小店的数据作为考核
			comId = "ALL";
		}
		String ym = beginYmd.substring(0, 7);
		BigDecimal distributionValue = comProgressDao.getComDistributionCnf(storeId, groupId, comId);// 小店分配比例
		...
		if (DateUtil.getBetweenDays(endYmd, DateUtil.getDate("yyyy-MM-dd")) == 0) {// 只有结束时间包含当天才需要累计当天的销售
			todayDeptSaleList = comProgressDao.getTodayDeptSale(storeId, comId);// 当日小店所有大类实时销售
		}
		List<BaseReportGpExt> historyDeptSaleList = comProgressDao.getHistoryDeptSale(storeId, comId, beginYmd, endYmd);// 小店下所有大类历史销售
		List<GoalValue> goalValueList = goalDao.getComGPGoal(storeId, comId, ym);// 整月小店各项目标
		List<IncomeConfig> incomeCnfList = commonDao.getAllIncomeCnfByStoreId(ym, storeId);// 获取门店的后台收入率列表
		List<DeptStock> deptStockList = comProgressDao.getDeptStock(storeId, comId,
				DateUtil.getCurMonthYesterday(endYmd));// 大类库存
		List<DeptInfo> deptInfoList = deptInfoDao.getDeptInfoByCom(storeId, comId);
		List<TurnOverDays> turnOverDaysList = comProgressDao.getDeptTurnoverDays(storeId, ym);// 标准周转天数
		ProgressCoreData data = this.getProgressCoreData(beginYmd, endYmd, todayDeptSaleList, historyDeptSaleList,
				goalValueList, incomeCnfList, deptStockList, turnOverDaysList, distributionValue, deptInfoList);
	 ...
	 * @throws Exception
	 */
	public ShareDetail getProgressReportDetail(String groupId, String storeId, String comId, String beginYmd,
			String endYmd) throws Exception {
		if (StringUtils.isEmpty(comId)) {// 当大店进入的时候以后勤小店的数据作为考核
			comId = "ALL";
		}
		String ym = beginYmd.substring(0, 7);
		BigDecimal distributionValue = comProgressDao.getComDistributionCnf(groupId, comId, storeId);// 小店分配比例
		List<BaseReportGpExt> todayDeptSaleList = null;
		if (DateUtil.getBetweenDays(endYmd, DateUtil.getDate("yyyy-MM-dd")) == 0) {// 只有结束时间包含当天才需要累计当天的销售
			todayDeptSaleList = comProgressDao.getTodayDeptSale(storeId, comId);// 当日小店所有大类实时销售
		}
		List<BaseReportGpExt> historyDeptSaleList = comProgressDao.getHistoryDeptSale(storeId, comId, beginYmd, endYmd);// 小店下所有大类历史销售
		List<GoalValue> goalValueList = goalDao.getComGPGoal(storeId, comId, ym);// 整月小店各项目标
		List<IncomeConfig> incomeCnfList = commonDao.getAllIncomeCnfByStoreId(ym, storeId);// 获取后台收入率列表
		List<DeptStock> deptStockList = comProgressDao.getDeptStock(storeId, comId,
				DateUtil.getCurMonthYesterday(endYmd));// 大类库存
		List<TurnOverDays> turnOverDaysList = comProgressDao.getDeptTurnoverDays(storeId, ym);// 标准周转天数
		List<DeptInfo> deptInfoList = deptInfoDao.getDeptInfoByCom(storeId, comId);
	...
	public List<Accounting> getAccountingList(String storeId, String comId) throws Exception {
		if (StringUtils.isEmpty(comId)) {// 当大店进入的时候以后勤小店的数据作为考核
			comId = "ALL";
		}
		List<AssessResult> assessResultList = comAssessDao.getLastYearResult(storeId, comId);
	...
	public AccountingItemResult getAccountingItem(String groupId, String storeId, String comId, String beginYmd,
			String endYmd) throws Exception {
		if (StringUtils.isEmpty(comId)) {// 当大店进入的时候以后勤小店的数据作为考核
			comId = "ALL";
		}
		...
		if (DateUtil.isCurMonth(beginYmd)) {
			...
			List<BaseReportGpExt> historyDeptSaleList = comProgressDao.getHistoryDeptSale(storeId, comId, beginYmd,
					endYmd);// 小店下所有大类历史销售
			List<GoalValue> goalValueList = goalDao.getComGPGoal(storeId, comId, ym);// 整月小店各项目标
			List<IncomeConfig> incomeCnfList = commonDao.getAllIncomeCnfByStoreId(ym, storeId);// 获取后台收入率列表
			BigDecimal distributionValue = comProgressDao.getComDistributionCnf(groupId, comId, storeId);// 小店分配比例
			...
			List<AssessResult> assessResultList = comAssessDao.getResultByYm(storeId, comId, ym);
