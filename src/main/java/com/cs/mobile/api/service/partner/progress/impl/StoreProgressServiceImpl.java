...
import com.cs.mobile.api.dao.partner.assess.StoreAssessDao;
import com.cs.mobile.api.dao.partner.goal.GoalDao;
import com.cs.mobile.api.dao.partner.progress.StoreProgressDao;
...
@Service
public class StoreProgressServiceImpl extends ProgressAbstractService implements StoreProgressService {
	@Autowired
	CommonDao commonDao;
	@Autowired
	GoalDao goalDao;
	@Autowired
	StoreProgressDao storeProgressDao;
	@Autowired
	...
	public ProgressReportResult getProgressReport(String areaId, String beginYmd, String endYmd) throws Exception {
		...
		List<DistributionValues> dValueList = storeProgressDao.getComDistributionCnf(areaId);// 小店分配比例
		...
		if (DateUtil.getBetweenDays(endYmd, DateUtil.getDate("yyyy-MM-dd")) == 0) {// 只有结束时间包含当天才需要累计当天的销售
			todayDeptSaleList = storeProgressDao.getTodayDeptSale(areaId);// 当日小店所有大类实时销售
		}
		List<BaseReportGpExt> historyDeptSaleList = storeProgressDao.getHistoryDeptSale(areaId, beginYmd, endYmd);// 小店下所有大类历史销售
		List<GoalValue> goalValueList = goalDao.getStoreGPGoal(areaId, ym);// 整月小店各项目标
		List<IncomeConfig> incomeCnfList = commonDao.getAllIncomeCnfByAreaId(ym, areaId);// 获取后台收入率列表
		List<DeptStock> deptStockList = storeProgressDao.getDeptStock(areaId, DateUtil.getCurMonthYesterday(endYmd));// 大类库存
		List<TurnOverDays> turnOverDaysList = storeProgressDao.getDeptTurnoverDays(areaId, ym);// 标准周转天数
		List<DeptInfo> deptInfoList = deptInfoDao.getDeptInfoByArea(areaId);
	...
	public List<Accounting> getAccountingList(String areaId) throws Exception {
		List<AssessResult> assessResultList = storeAssessDao.getLastYearResult(areaId);
	...
	public AccountingItemResult getAccountingItem(String areaId, String beginYmd, String endYmd) throws Exception {
		...
		if (DateUtil.isCurMonth(beginYmd)) {
			...
			List<IncomeConfig> incomeCnfList = commonDao.getAllIncomeCnfByAreaId(ym, areaId);// 获取后台收入率列表
			List<DistributionValues> dValueList = storeProgressDao.getComDistributionCnf(areaId);// 小店分配比例
			List<DeptStock> deptStockList = storeProgressDao.getDeptStock(areaId,
			...
			List<AssessResult> assessResultList = storeAssessDao.getResultByYm(areaId, ym);
